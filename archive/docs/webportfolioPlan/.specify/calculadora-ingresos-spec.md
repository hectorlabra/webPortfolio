...
- Q: ¿Qué criterios de aceptación medibles definen el éxito de la calculadora? → A: Todas las anteriores (métricas completas de éxito) - Incluye: completar cálculo en <2 min, Lighthouse >90, tiempo en página >3 min, tasa de engagement >80%, click-through rate >15%
- Q: ¿Cómo manejar estados de error y edge cases en la calculadora? → A: Todas las anteriores (manejo completo de errores) - Incluye: validación en tiempo real con mensajes contextuales, límites razonables en inputs, valores por defecto inteligentes, recuperación automática de errores, estados de carga con skeletons, mensajes de error con sugerencias, tooltips explicativos, ejemplos pre-cargados
- Q: ¿Qué requisitos de seguridad y compliance aplicar a la calculadora? → A: Todas las anteriores (seguridad completa) - Incluye: almacenamiento local únicamente sin envío a servidor, HTTPS obligatorio, compliance GDPR con consentimiento de cookies, protección XSS con sanitización, validación de inputs, rate limiting, Content Security Policy
